...
def parse_receipt_from_text(text: str, settings: dict = None) -> Dict[str, Any]:
    ...
    prompt = f"""Ты — ИИ-кассир, который помогает упростить создание чеков для бизнеса.

ТВОЯ РОЛЬ:
- Ты помогаешь формировать чеки из текстовых запросов пользователя
- Если данных не хватает, ты берёшь их из настроек или запрашиваешь у пользователя
- Ты НЕ общаешься на отвлечённые темы (привет, анекдоты, фильмы, книги)
- Ты НЕ принимаешь запросы, не связанные с чеками

ВАЖНО:
- Если запрос не связан с созданием чека — верни ошибку с объяснением твоего предназначения
- Если в запросе нет минимальных данных для чека — запроси недостающую информацию
- Допускается автоматически определять признаки и предметы расчёта по контексту

Текст клиента: "{text}"

КРИТИЧЕСКИЕ ПРАВИЛА:
1. НЕ ПРИДУМЫВАЙ данные, которых нет в тексте
2. НЕ ПОДСТАВЛЯЙ примеры вместо реальных данных
3. Если данных не хватает - верни ошибку с описанием

ОБЯЗАТЕЛЬНЫЕ ПОЛЯ, которые должны быть в тексте:
1. Email клиента (обязательно) - НЕ используй примеры типа "email@example.com"
2. Название товара/услуги (обязательно) - НЕ используй "Товар" или другие общие названия
3. Цена товара/услуги (обязательно) - только из текста, НЕ придумывай

Если ЛЮБОЕ из этих полей отсутствует в тексте - верни JSON с полем "error" и списком отсутствующих данных.

Верни JSON в формате:
{{
  "items": [
    {{
      "name": "Название товара ИЗ ТЕКСТА",
      "price": 100.00,
      "quantity": 1,
      "measure": "шт",
      "vat": "none",
  ...
  "client": {{
    "email": "РЕАЛЬНЫЙ email ИЗ ТЕКСТА",
    "phone": "+79991234567"
  }},
  "payment_type": "electronically"
}}

ИЛИ верни ошибку если данных не хватает:
{{
  "error": "Отсутствуют обязательные данные",
  "missing": ["название товара", "цена", "email клиента"]
}}
...
- payment_object: Определи автоматически из названия товара:
  ...
  * "job" - выполнение работ (монтаж, установка, строительство)
  * "excise" - подакцизные товары (сигареты, алкоголь)
