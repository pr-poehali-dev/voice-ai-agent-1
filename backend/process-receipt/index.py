...
def parse_receipt_from_text(text: str, settings: dict = None) -> Dict[str, Any]:
    ...
    prompt = f"""Ты — ИИ-парсер чеков. Извлекаешь данные из текста пользователя для создания кассового чека.

ЗАПРОС ПОЛЬЗОВАТЕЛЯ: "{text}"

ТВОЯ ЗАДАЧА:
Извлечь из запроса данные для чека и вернуть ТОЛЬКО валидный JSON.

ПРАВИЛА ИЗВЛЕЧЕНИЯ НАЗВАНИЯ ТОВАРА/УСЛУГИ:
- ❌ НЕ включай команды: "создай", "пробей", "отправь", "сделай"
- ❌ НЕ включай предлоги: "на", "для", "чек"
- ✅ Извлекай ТОЛЬКО название товара/услуги
- Примеры:
  * "создай чек на кофе 200 руб" → name: "Кофе"
  * "пробей круассан 150р" → name: "Круассан"
  * "чек на стрижку 1500" → name: "Стрижка"
  * "отправь чек доставка пиццы 500" → name: "Доставка пиццы"

ТИПЫ ОПЕРАЦИЙ (operation_type):
- "sell" - продажа (по умолчанию)
- "sell_refund" - возврат товара
- "sell_correction" - коррекция продажи

Определяй по словам: "возврат", "корректировка", "исправление"

ТИПЫ ОПЛАТЫ (payment_type):
- "cash" - наличные (если указано: "наличными", "наличка", "кэш")
- "electronically" - безналичные (по умолчанию)
- "prepaid" - предоплата/аванс (если указано: "аванс", "предоплата")
- "credit" - постоплата/кредит (если указано: "в долг", "кредит", "постоплата")

ПРЕДМЕТ РАСЧЁТА (payment_object):
- "commodity" - товар (кофе, круассан, телефон, книга)
- "service" - услуга (консультация, стрижка, доставка, клининг)
- "job" - работа (монтаж, установка, ремонт, строительство)
- "excise" - подакцизный товар (алкоголь, сигареты)

СПОСОБ РАСЧЁТА (payment_method):
- "full_payment" - полная оплата (по умолчанию)
- "prepayment" - предоплата
- "advance" - аванс
- "partial_payment" - частичная оплата

НДС (vat):
- "none" - без НДС (по умолчанию)
- "vat20" - НДС 20%
- "vat10" - НДС 10%

ЕДИНИЦА ИЗМЕРЕНИЯ (measure):
- "шт" - штуки (по умолчанию для товаров)
- "услуга" - для услуг
- "работа" - для работ
- "кг", "л", "м" - если указано

ОБЯЗАТЕЛЬНЫЕ ДАННЫЕ:
1. Название товара/услуги (БЕЗ команд "создай", "пробей")
2. Цена
3. Email клиента

Если чего-то не хватает - верни JSON с ошибкой.
...
{{
  ...
  "items": [
    {{
      "name": "Название товара БЕЗ команд",
      "price": 100.00,
      "quantity": 1,
      "measure": "шт",
      "vat": "none",
...
ИЛИ если данных не хватает:
{{
  "error": "Недостаточно данных для создания чека",
  "missing": ["email клиента"]
}}
...
Твой ответ (ТОЛЬКО JSON):"""
    ...
    for attempt in range(max_retries):
        try:
            timeout = 12 if attempt == 0 else 18
